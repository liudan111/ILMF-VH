...
class NRLMF:
    ...
    def log_likelihood(self):
        ...
        #print "lambda_d:%.6f,2:%.6f,3:%.6f,4:%.6f\n" % (self.lambda_d,np.sum(np.square(self.U)),self.lambda_t,np.sum(np.square(self.V))
        #print "self.alpha:%.6f,2:%.6f,3:%.6f,4:%.6f\n" % (self.alpha,self.U.T,self.DL,self.U)
    ...
    def predict_scores(self, test_data, N,seq):
        ...
        for d, t in test_data:
            if d in self.train_drugs:
                ...
                if t in self.train_targets:
                    ...
                    val = np.sum(v1*v2)
            scores.append(np.exp(val+seq[d,t])/(1+np.exp(val+seq[d,t])))
    ...
    def evaluation(self, test_data, test_label):
        ...
        auc_val = auc(fpr, tpr)
        return np.array(fpr, dtype=np.float64),np.array(tpr, dtype=np.float64),aupr_val, auc_val
